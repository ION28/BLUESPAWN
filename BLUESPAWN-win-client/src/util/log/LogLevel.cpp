...
namespace Log {
    ...
    LogLevel::LogLevel(IN Severity severity, IN CONST std::optional<Detail>& detail OPTIONAL) :
        enabled{ true }, severity{ severity }, detail{ detail } {}
    LogLevel::LogLevel(IN Severity severity, IN bool DefaultState, IN CONST std::optional<Detail>& detail OPTIONAL) :
        enabled{ enabled }, severity{ severity }, detail{ detail } {}
    ...
    LogLevel::operator int() const { return static_cast<int>(severity); }
}   // namespace Log
