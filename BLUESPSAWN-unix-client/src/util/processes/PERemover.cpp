...
PERemover::PERemover(const HandleWrapper& hProcess, void* lpBaseAddress, unsigned int dwImageSize) : 
    hProcess{ hProcess }, 
...
PERemover::PERemover(unsigned int dwPID, void* lpBaseAddress, unsigned int dwImageSize) :
    PERemover(OpenProcess(PROCESS_ALL_ACCESS, false, dwPID), lpBaseAddress, dwImageSize){}
PERemover::PERemover(const HandleWrapper& hProcess, const std::string& wsImageName) :
    PERemover(hProcess, GetModuleHandleW(wsImageName.c_str()), -1){}
PERemover::PERemover(unsigned int dwPID, const std::string& wsImageName) :
...
bool PERemover::AddressIsInRegion(void* lpAddress){
...
bool PERemover::CheckThreads(){
    ...
    if(!hThreadSnapshot){
        LOG_ERROR("Unable to open Tool Help Snapshot entry to scan threads" << " (Error " << errno << ")");
        return false;
    }
    ...
    if(!Thread32First(hThreadSnapshot, &ThreadEntry)){
        LOG_ERROR("Unable to open thread  entry to scan threads" << " (Error " << errno << ")");
        return false;
    }
    ...
    do if(ThreadEntry.th32OwnerProcessID == PID){
        ...
        if(!hThread){
            LOG_ERROR("Unable to open thread with TID " << ThreadEntry.th32ThreadID << " to scan for infected memory (Error " << errno << ")");
            return false;
        }
...
bool PERemover::WalkThreadBack(const HandleWrapper& hThread, unsigned int dwTID){
    CONTEXT context{};
    ...
    if(AddressIsInRegion(reinterpret_cast<void*>(context.Rip))){
        if(TerminateThread(hThread, 0)){
            LOG_INFO("Thread with TID " << dwTID << " was executing malicious code at " << reinterpret_cast<void*>(context.Rip) << " and was terminated");
            return true;
        } else {
            LOG_ERROR("Thread with TID " << dwTID << " was executing malicious code at " << reinterpret_cast<void*>(context.Rip) << " but couldn't terminated" << " (Error " << errno << ")");
            return false;
        }
    }
#else
    ...
    if(AddressIsInRegion(reinterpret_cast<void*>(context.Eip))){
        if(TerminateThread(hThread, 0)){
            LOG_INFO("Thread with TID " << dwTID << " was executing malicious code at " << reinterpret_cast<void*>(context.Eip) << " and was terminated");
            return true;
        } else {
            LOG_ERROR("Thread with TID " << dwTID << " was executing malicious code at " << reinterpret_cast<void*>(context.Eip) << " but couldn't terminated");
            return false;
        }
    }
#endif
    ...
    while(StackWalk64(dwMachineType, hProcess , hThread, &stack, &context, nullptr, SymFunctionTableAccess64, SymGetModuleBase64, nullptr)){
        if(AddressIsInRegion(reinterpret_cast<void*>(stack.AddrPC.Offset))){
            if(TerminateThread(hThread, 0)){
                LOG_INFO("Thread with TID " << dwTID << " was executing malicious code at " << reinterpret_cast<void*>(stack.AddrPC.Offset) << " and was terminated");
                return true;
            } else {
                LOG_ERROR("Thread with TID " << dwTID << " was executing malicious code at " << reinterpret_cast<void*>(stack.AddrPC.Offset) << " but couldn't terminated" << " (Error " << errno << ")");
                return false;
            }
        }
    }
...
unsigned int GetFunctionStackSize(void* lpFunction, const HandleWrapper& hProcess, const HandleWrapper& hThread){
    CONTEXT context{};
...
bool PERemover::AdjustPointer(void* lpAddress){
    ...
    if(!VirtualQueryEx(hProcess, lpAddress, &memory, sizeof(memory))){
        ...
        if(memory.Protect & 0xF0){
            ...
            unsigned int dwOldProtections{};
            if(!VirtualProtectEx(hProcess, lpAddress, 4, PAGE_READWRITE, &dwOldProtections)){
                LOG_ERROR("Unable to adjust memory protections at " << lpAddress << " (Error " << errno << ")");
                return false;
            }
            ...
            if(!VirtualProtectEx(hProcess, lpAddress, 4, dwOldProtections, &dwOldProtections)){
                LOG_ERROR("Unable to repair memory protections at " << lpAddress << " (Error " << errno << ")");
                return false;
            }
        } else {
        ...
        if(!VirtualQueryEx(hProcess, reinterpret_cast<void*>(address), &memory, sizeof(memory))){
            ...
            if(ReadProcessMemory(hProcess, memory.BaseAddress, buffer, memory.RegionSize, nullptr)){
                for(int i = 0; i < memory.RegionSize - sizeof(ULONG_PTR); i++){
                    auto lpCheckAddress = *reinterpret_cast<void**>(reinterpret_cast<ULONG_PTR>(void*(buffer)) + i);
                    if(AddressIsInRegion(lpCheckAddress)){
                        if(!AdjustPointer(lpCheckAddress)){
                            LOG_ERROR("Unable to adjust pointer to bad memory at " << address + i << " (Error " << errno << ")");
                            return false;
                        }
                    }
                }
...
bool PERemover::WipeMemory(){
    ...
    if(!ReadProcessMemory(hProcess, lpBaseAddress, headers, 0x1000, nullptr)){
        LOG_ERROR("Unable to read memory to wipe at " << lpBaseAddress << " (Error " << errno << ")");
        return false;
    }

    if(headers[0] == 'M' && headers[1] == 'Z'){
        unsigned int dwNTHeaderOffset = reinterpret_cast<PIMAGE_DOS_HEADER>(void*(headers))->e_lfanew;

        unsigned int dwEntrypointRVA{};
        unsigned int dwExportsRVA{};
        unsigned int dwExportsSize{};

        if(dwNTHeaderOffset + sizeof(IMAGE_NT_HEADERS64) < 0x1000){
            auto lpNTHeaders = reinterpret_cast<PIMAGE_NT_HEADERS>(reinterpret_cast<PCHAR>(void*(headers)) + dwNTHeaderOffset);
            ...
            auto lpNTHeaders = reinterpret_cast<PIMAGE_NT_HEADERS>(void*(NTHeaders));
            if(lpNTHeaders->Signature != 0x00004550){
                return true;
            }
        ...
        if(dwExportsRVA && dwExportsSize){
            ...
            auto lpExports = reinterpret_cast<PIMAGE_EXPORT_DIRECTORY>(void*(Exports));
            auto dwFunctions = lpExports->NumberOfFunctions;
            auto lpFuncionsPtr = reinterpret_cast<Punsigned int>(reinterpret_cast<ULONG_PTR>(void*(Exports)) + lpExports->AddressOfFunctions - dwExportsRVA);

            for(unsigned int i = 0; i < dwFunctions; i++){
