...
private:
	...
	class SubEventListener {
		...
		std::vector<Events::EventHandle> events;

		/// Maps events to callbacks to be run when the event is triggered
		std::map<Events::EventHandle, std::vector<std::function<void()>>> map;
		...
		void HandleEventNotify(Events::EventHandle);
