...
std::string FormatStatTime(const struct tm * ft){
		std::stringstream w;
		w << std::setfill('0') << std::to_string(ft->tm_year) << "-" << std::setw(2) << std::to_string(ft->tm_mon) << "-" << std::setw(2) << std::to_string(ft->tm_mday) << " " <<
			std::setw(2) << std::to_string(ft->tm_hour) << ":" << std::setw(2) << std::to_string(ft->tm_min) << ":" << std::setw(2) << std::to_string(ft->tm_sec) << "." << "Z";
	return w.str();
}