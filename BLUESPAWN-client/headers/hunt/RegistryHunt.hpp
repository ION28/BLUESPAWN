...
namespace Registry {
	 ...
	 */
	template<class T>
	inline bool CheckKey(RegistryKey key, T value, Reaction& reaction, MatchAction bOnMatch = MatchAction::NO_MATCH_BAD){
		bool equal = key.Get<T>() == value;

		if(!equal && bOnMatch == MatchAction::NO_MATCH_BAD || equal && bOnMatch == MatchAction::MATCH_BAD){
			LOG_WARNING("Potentially bad registry key " << key << " with value \"" << key.Get<T>() << "\". Value should " << (bOnMatch == MatchAction::NO_MATCH_BAD ? "" : "not ") << "be \"" << value << "\"");
	 ...
	 */
	template<class T>
	inline bool CheckKey(RegistryKey key, std::vector<T> values, Reaction& reaction, MatchAction bOnMatch = MatchAction::NO_MATCH_BAD){
		...
		if(bOnMatch == NO_MATCH_BAD && matched){
			...
			LOG_WARNING("Potentially bad registry key " << key << " with value \"" << key.Get<T>() << "\". Value should " << (bOnMatch == MatchAction::NO_MATCH_BAD ? "" : "not ") << "be " << stream << "");
		}
	...
	inline bool CheckKey(RegistryKey key, REG_MULTI_SZ_T values, Reaction& reaction, MatchAction bOnMatch){
		...
		for(auto value : key.Get<REG_MULTI_SZ_T>()){
			...
			if(inList && bOnMatch == MatchAction::MATCH_BAD || !inList && bOnMatch == MatchAction::NO_MATCH_BAD) {
