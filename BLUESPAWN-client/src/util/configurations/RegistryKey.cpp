...
namespace Registry {
    ...
    std::map<HKEY, std::wstring> vHives{
        { HKEY_LOCAL_MACHINE, L"HKEY_LOCAL_MACHINE" },   { HKEY_CLASSES_ROOT, L"HKEY_CLASSES_ROOT" },
        { HKEY_CURRENT_USER, L"HKEY_CURRENT_USER" },     { HKEY_USERS, L"HKEY_USERS" },
    ...
    RegistryKey::RegistryKey(const RegistryKey& key) noexcept :
        bKeyExists{ key.bKeyExists }, bWow64{ key.bWow64 }, hkBackingKey{ key.hkBackingKey } {
    ...
    RegistryKey::RegistryKey(HKEY key) : bKeyExists{ true }, bWow64{ false }, hkBackingKey{ key } {
        if(_ReferenceCounts.find(hkBackingKey) == _ReferenceCounts.end()) {
            _ReferenceCounts[hkBackingKey] = 1;
        } else {
            _ReferenceCounts[hkBackingKey]++;
    ...
    RegistryKey::RegistryKey(HKEY hive, std::wstring path, bool WoW64) {
        ...
        LSTATUS status = RegOpenKeyEx(hive, path.c_str(), 0,
                                      KEY_ALL_ACCESS | (bWow64 ? KEY_WOW64_32KEY : KEY_WOW64_64KEY), &hkBackingKey);
        if(status == ERROR_ACCESS_DENIED) {
            status = RegOpenKeyEx(hive, path.c_str(), 0,
                                  KEY_READ | KEY_NOTIFY | (bWow64 ? KEY_WOW64_32KEY : KEY_WOW64_64KEY), &hkBackingKey);
        }
    ...
    RegistryKey::RegistryKey(std::wstring name, bool WoW64) {
        ...
        if(vHiveNames.find(HiveName) == vHiveNames.end()) {
            ...
            if(slash == name.length()) {
                ...
                bWow64 = WoW64 || wLowerPath.find(L"wow6432node") != std::wstring::npos;
                LSTATUS status = RegOpenKeyEx(hkHive, path.c_str(), 0,
                                              KEY_ALL_ACCESS | (WoW64 ? KEY_WOW64_32KEY : KEY_WOW64_64KEY),
                                              &hkBackingKey);
                if(status == ERROR_ACCESS_DENIED) {
                    status = RegOpenKeyEx(hkHive, path.c_str(), 0,
                                          KEY_READ | KEY_NOTIFY | (WoW64 ? KEY_WOW64_32KEY : KEY_WOW64_64KEY),
                                          &hkBackingKey);
                }
    ...
    bool RegistryKey::Exists() const { return bKeyExists; }
    ...
    bool RegistryKey::Create() {
        ...
        LSTATUS status =
            RegCreateKeyEx(hkHive, path.c_str(), 0, nullptr, 0, KEY_ALL_ACCESS, nullptr, &hkBackingKey, nullptr);
        if(status == ERROR_SUCCESS) {
            bKeyExists = true;
    ...
    std::optional<T> RegistryKey::GetValue(const std::wstring& wsValueName) const {
        if(ValueExists(wsValueName)) { return GetRawValue(wsValueName).Dereference<T>(); }
        return std::nullopt;
    }
    ...
    template bool
    RegistryKey::SetValue<std::wstring>(const std::wstring& name, std::wstring value, DWORD size, DWORD type) const;
    ...
    template bool
    RegistryKey::SetValue<std::string>(const std::wstring& name, std::string value, DWORD size, DWORD type) const;

    template<>
    bool
    RegistryKey::SetValue(const std::wstring& name, std::vector<std::wstring> value, DWORD _size, DWORD type) const {
        SIZE_T size = 1;
        for(auto string : value) { size += (string.length() + 1); }

        auto data = new WCHAR[size];
        auto allocation =
            AllocationWrapper{ data, static_cast<DWORD>(size * sizeof(WCHAR)), AllocationWrapper::CPP_ARRAY_ALLOC };
        unsigned ptr = 0;

        for(auto string : value) {
            LPCWSTR lpRawString = string.c_str();
            for(unsigned i = 0; i < string.length() + 1; i++) {
                if(ptr < size) { data[ptr++] = lpRawString[i]; }
            }
        }

        if(ptr < size) { data[ptr] = { static_cast<WCHAR>(0) }; }
    ...
    template bool RegistryKey::SetValue<std::vector<std::wstring>>(const std::wstring& name,
    ...
    std::vector<RegistryKey> RegistryKey::EnumerateSubkeys() const {
        ...
        LSTATUS status = RegQueryInfoKey(hkBackingKey, nullptr, nullptr, 0, &dwSubkeyCount, &dwLongestSubkey, nullptr,
        ...
        if(status == ERROR_SUCCESS && dwSubkeyCount) {
            for(unsigned i = 0; i < dwSubkeyCount; i++) {
                ...
                if(status == ERROR_SUCCESS) { vSubKeys.push_back({ hkBackingKey, lpwName }); }
    ...
    std::vector<std::wstring> RegistryKey::EnumerateValues() const {
        ...
        if(status == ERROR_SUCCESS && dwValueCount) {
            for(unsigned i = 0; i < dwValueCount; i++) {
                ...
                if(status == ERROR_SUCCESS) { vSubKeys.push_back({ lpwName }); }
        ...
        if(hkBackingKey && Linker::NtQueryKey) {
            ...
            if(result == ((NTSTATUS) 0xC0000023L)) {
                ...
                if(buffer != NULL) {
                    ...
                    if(location != std::string::npos) { keyPath.replace(location, 17, L"HKEY_LOCAL_MACHINE"); }
                    location = keyPath.find(L"\\REGISTRY\\USER");
                    if(location != std::string::npos) { keyPath.replace(location, 14, L"HKEY_USERS"); }
                }
            }
        }
        return keyPath;
    ...
    std::wstring RegistryKey::ToString() const { return GetName(); }

    bool RegistryKey::operator==(const RegistryKey& key) const { return hkBackingKey == key.hkBackingKey; }
