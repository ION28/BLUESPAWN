...
std::unordered_map<std::reference_wrapper<Detection>, Association> ServiceScanner::GetAssociatedDetections(
	...
	if(data.FilePath){
		detections.emplace(Detection{ FileDetectionData{*data.FilePath }, std::nullopt, std::nullopt,
						   detection.DetectionStale }, Certainty::Certain);
	}
	...
	if(!detection.DetectionStale){
		...
		if(!data.ServiceName && data.DisplayName){
			GenericWrapper<SC_HANDLE> ServiceManager{ OpenSCManagerW(nullptr, nullptr, GENERIC_READ), CloseServiceHandle };
			...
			while(!bfsQueue.empty()){
				...
				if(visited.find(key) == visited.end()){
					...
					for(auto name : key.EnumerateValues()){
						...
						if(val && val->GetType() == RegistryType::REG_SZ_T || val->GetType() == RegistryType::REG_EXPAND_SZ_T){
							...
							if(str.find(L".exe") || str.find(L".dll")){
								detections.emplace(Bluespawn::detections.AddDetection(Detection{
									RegistryDetectionData{ key, val, RegistryDetectionType::FileReference }
								}), Association::Strong);
							}
						}
					}
